defmodule GothamManager.Api do
  @moduledoc """
  The Api context.
  """

  import Ecto.Query, warn: false
  alias GothamManager.Repo
  ...
  def get_by_param(email, username) do
  ...
  def get_clock_by_user(user_id), do: Repo.all(Clock, user_id: user_id)
  ...
  def get_one_workingtime_by_user_id(user_id, id) do
     Repo.get_by(Workingtime, user_id: user_id, id: id)
  end

  def get_all_workingtimes_from_user(user_id, end_value, start_value) do
    query = from(w in Workingtime, where: w.user_id == ^user_id and w.start >= ^start_value and w.end <= ^end_value)
    Repo.all(query)
  end
